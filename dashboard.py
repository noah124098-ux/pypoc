...
conn = _db_connect()
tab_live, tab_portfolio, tab_pnl, tab_positions, tab_replay, tab_regime, tab_backtest, tab_ai_review, tab_controls, tab_costs = st.tabs([
    "🔴 Paper Agent",
    "📊 Angel One Account",
    "📊 P&L & Equity",
    "📋 Positions & Signals",
...
with tab_replay:
    ...
    _rpl_duration_min = getattr(_replay, "frames", None)  # placeholder; we compute directly
    try:
        from datetime import datetime as _dt_cls
        _rpl_opened_dt = _dt_cls.fromisoformat(str(_replay.entry_time).replace(" ", "T")[:19])
        _rpl_closed_dt = _dt_cls.fromisoformat(str(_replay.exit_time).replace(" ", "T")[:19])
    ...
    if _df_chart is not None:
        # Filter to trade window +/- 5 trading days
        try:
            ...
            if not _df_window.empty:
                ...
                # Horizontal lines: entry (blue), stop (red dashed), target (green dashed)
                ...
                _df_chart = None  # fall through to simple chart
        except Exception:
            _df_chart = None

    if _df_chart is None:
        # Simple line chart from entry to exit price
    ...
    # Mid-trade position value (approximate — use midpoint price)
    _tl_mid_price = (_rpl_entry + _rpl_exit) / 2
    _tl_mid_unr = (_tl_mid_price - _rpl_entry) * _rpl_qty
    _tl_mid_time = ""
    try:
        from datetime import datetime as _dt2
        _open2 = _dt2.fromisoformat(_tl_entry_time[:19].replace(" ", "T"))
        _close2 = _dt2.fromisoformat(_tl_exit_time[:19].replace(" ", "T"))
    ...
    if _tl_mid_time and _tl_mid_time != "—":
        _tl_mid_color = "green" if _tl_mid_unr >= 0 else "red"
        st.markdown(
